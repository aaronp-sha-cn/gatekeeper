...
# 由 preseed late_command 在 Debian 13 (Trixie) 安装过程中调用
...
# - Debian 13 (Trixie) 使用 DEB822 格式 sources 文件
# ============================================================
...
rm -f /target/etc/apt/sources.list.d/debian-cdrom.list 2>/dev/null || true
# 移除所有包含 cdrom 的 .sources 文件（DEB822 格式）
rm -f /target/etc/apt/sources.list.d/*cdrom*.sources 2>/dev/null || true
...
rm -f /etc/apt/sources.list.d/debian-cdrom.list 2>/dev/null || true
rm -f /etc/apt/sources.list.d/*cdrom*.sources 2>/dev/null || true
echo "[GateKeeper] [0] CD-ROM apt 源已禁用"

# ============================================================
# 1. 配置 apt 源（Debian 13 Trixie - DEB822 格式）
#    必须在 postinstall.sh 之前配置，确保 chroot 中 apt-get 可用
#    Debian 13 使用 /etc/apt/sources.list.d/debian.sources (DEB822)
# ============================================================
echo "[GateKeeper] [1] 配置 apt 源（DEB822 格式）..."
mkdir -p /target/etc/apt/sources.list.d
cat > /target/etc/apt/sources.list.d/debian.sources << 'APT_EOF'
Types: deb deb-src
...
Components: main contrib non-free non-free-firmware
APT_EOF
echo "[GateKeeper] [1] apt 源已配置（DEB822 格式 - trixie）"
